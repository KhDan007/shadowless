...
export function HintStrip({
  ...
  let key = "idle";
  let icon = <Lightbulb size={12} className="text-[color:var(--risk-medium)]" />;
  let body: React.ReactNode = (
    <>
      <span className="text-foreground/80">Tip — select a high-risk node to begin investigation.</span>
      <span className="mono ml-1 text-[11px] text-muted-foreground">3 critical nodes</span>
    </>
  );
