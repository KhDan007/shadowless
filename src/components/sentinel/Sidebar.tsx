...
export function Sidebar({ collapsed = false, onNavigate }: { collapsed?: boolean; onNavigate?: () => void }) {
  ...
  if (collapsed) {
    return (
      <TooltipProvider delayDuration={200}>
        <aside className="flex h-full w-14 shrink-0 flex-col items-center border-r border-[#2a2a2a] bg-[#080808] py-3">
          <div className="relative flex h-9 w-9 items-center justify-center rounded bg-gradient-to-br from-[#ffb000] to-[#b87f00]">
            ...
            <span className="absolute -bottom-0.5 -right-0.5 h-2 w-2 rounded-full bg-[#ffc94d] ring-2 ring-[#080808]" />
          </div>
  ...
  return (
    <aside className="flex h-full w-full min-w-[200px] shrink-0 flex-col border-r border-[#2a2a2a] bg-[#080808]">
      {/* Brand */}
      <div className="flex items-center gap-2.5 border-b border-[#2a2a2a] px-4 py-3">
        ...
        <div className="flex flex-col leading-tight">
          <span className="text-[14px] font-bold tracking-wide text-[#e8e8e8]">SHADOWLESS</span>
          <span className="mono text-[10px] tracking-[0.15em] text-[#8a8a8a]" title="Ministry of Internal Affairs, Republic of Kazakhstan — Cybercrime Investigation Bureau, Unit 04">MIA · KZ · CIB-04</span>
        </div>
      </div>

      {/* Nav */}
      <nav className="px-2 py-3">
        <div className="px-2 pb-1.5 text-[11px] font-bold tracking-[0.14em] text-[#8a8a8a]">WORKSPACE</div>
        {NAV.map((item) => {
          const Icon = item.icon;
          const active = isActive(item.to);
          return (
      ...
      <div className="flex-1 overflow-y-auto px-2 pb-3">
        <div className="flex items-center justify-between px-2 pb-1.5 pt-3">
          <span className="text-[11px] font-bold tracking-[0.14em] text-[#8a8a8a]">ACTIVE CASE</span>
          <Popover>
            ...
            <PopoverContent side="right" align="start" className="w-72 border-[#2a2a2a] bg-[#111111] p-1.5">
              <div className="px-2 pb-1.5 pt-1 text-[11px] font-bold uppercase tracking-[0.14em] text-[#8a8a8a]">All cases</div>
                ...
                {CASES.map((c) => (
                  <button
                    ...
                    className={cn(
                      ...
                      c.id === activeCase ? "bg-[#2a1f00]" : "hover:bg-[#141414]",
                    )}
                  >
                    <div className="min-w-0">
                      <div className="mono text-[12px] font-semibold text-[#e8e8e8]">#{c.id}</div>
        ...
        <div className="rounded-sm border border-[#ffb000]/40 bg-[#2a1f00]/40 p-2.5">
          <div className="flex items-center justify-between">
            <span className="mono text-[12px] font-semibold text-[#e8e8e8]">#{selectedCase.id}</span>
            <RiskBadge risk={selectedCase.risk} />
          </div>
          <div className="mt-1 truncate text-[13px] text-[#e8e8e8]">{selectedCase.title}</div>
        ...
        <div className="mt-2 space-y-0.5">
          {CASES.filter((c) => c.id !== activeCase).slice(0, 3).map((c) => (
            <button
              key={c.id}
              onClick={() => setActiveCase(c.id)}
              className="flex w-full items-center gap-2 rounded-sm px-2 py-1.5 text-left text-[12.5px] text-[#b8b8b8] hover:bg-[#111111] hover:text-[#e8e8e8]"
            >
              <span className={cn(
              ...
              <span className="mono text-[11.5px] text-[#8a8a8a]">#{c.id}</span>
      ...
      <TooltipProvider delayDuration={120}>
        <div className="flex items-center justify-between gap-2 border-t border-[#2a2a2a] px-3 py-2">
          ...
          <span className="mono text-[11px] text-[#8a8a8a]">v2.4</span>
        </div>
      </TooltipProvider>
    </aside>
  );
...
function CaseStat({ label, value, hint }: { label: string; value: string; hint?: string }) {
  return (
    <div className="rounded-sm border border-[#2a2a2a] bg-[#0a0a0a] px-1 py-1" title={hint}>
      <div className="mono text-[13px] font-semibold text-[#e8e8e8]">{value}</div>
